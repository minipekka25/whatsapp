        ...
        <?php if(!is_product()): ?>
            ...
            <div class="wc-toolbar-left">
                ...
                <?php if(count($per_page_array) > 1): ?>
                    <div class="wc-view-count">
                        <p><?php echo esc_html_x('Show', 'front-view', 'lapa'); ?></p>
                        <ul><?php
                            foreach ($per_page_array as $val){?><li
                                <?php echo ($per_page == $val ? ' class="active"' : '')?>><a href="<?php echo esc_url(add_query_arg('per_page', $val, $current_url)); ?>"><?php echo sprintf( esc_html__( '%s' , 'lapa'), $val ) ?></a></li>
                            <?php }
                        ?></ul>
                    </div>
                <?php endif ;?>
            ...
            <div class="wc-toolbar-right">
                <?php if(Lapa()->settings()->get('woocommerce_toggle_grid_list','off') == 'on'): ?>
                    ...
                    ?>><i title="<?php echo esc_attr_x('Grid view', 'front-view', 'lapa') ?>" class="fa fa-th"></i></span>
                    <span data-view_mode="list"<?php
                    if ($view_mode == 'list') {
                        echo ' class="active"';
                    }
                    ?>><i title="<?php echo esc_attr_x('List view', 'front-view', 'lapa') ?>" class="fa fa-list"></i></span>
                    </div>
                <?php endif;?>
            ...
            <?php if ($active_shop_filter == 'on' && is_active_sidebar('sidebar-shop-filter')): ?>
                <div class="btn-advanced-shop-filter">
                    <span><?php echo esc_html_x('Filter', 'front-view', 'lapa'); ?></span><i></i>
                </div>
            <?php endif; ?>
        <?php endif; ?>
    </div><!-- .wc-toolbar -->
            ...
            <div class="la-advanced-product-filters clearfix">
                <div class="sidebar-inner clearfix">
                    ...
                    <div class="la-advanced-product-filters-result">
                        ...
                        ?>
                        <a class="reset-all-shop-filter text-color-primary" href="<?php echo esc_url($base_filter) ?>"><svg class="dlicon-close" width="26px" height="26px"><use xlink:href="#dlicon-close"></use></svg><span><?php echo esc_html_x('Clear All Filter', 'front-view', 'lapa'); ?></span></a>
                    </div>
                    <?php endif; ?>
                </div>
