...
<div id="la_compare_table_wrapper">
    ...
    $lists = Lapa_WooCommerce_Compare::get_data();

    $default_attr = lapa_get_wc_attribute_for_compare();
    $tax_attr = lapa_get_wc_attribute_taxonomies();
    $all_attr = array_merge($default_attr, $tax_attr);

    $attribute_allow = Lapa()->settings()->get('compare_attribute', $default_attr);
    ...
    if (!empty($lists)) {
        foreach ($lists as $product_id) {
            ...
            $term['id'] = $product_id;
            $term['sku'] = ($_product->get_sku() ? $_product->get_sku() : __('N/A', 'lapa'));
        ...
        <table class="la-compare-table">
            ...
            foreach( $attribute_allow as $attr_alow ){
                if(in_array($attr_alow, array('image','title','add-to-cart'))){
                    $label_list['info'] = esc_html_x('Product Info', 'front-view', 'lapa');
                }
                else{
                ...
                foreach ($terms as $item) {
                    ...
                    elseif ($k == 'stock') {
                        ...
                        if ($item['availability'] && !empty($item['availability']['availability'])) {
                            ...
                            if ($term['stock']) {
                                $text_stock = __('In stock', 'lapa');
                                $class_stock = 'in-stock';
                            } else {
                                $text_stock = __('Out of stock', 'lapa');
        ...
        ?>
        <p class="text-center"><?php esc_html_e('No products were added to the list', 'lapa') ?></p>
        <?php
    }
    ?>
</div>
